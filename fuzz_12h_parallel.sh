...
# Run all 6 targets in parallel for 12 hours simultaneously
#
# Total runtime: ~12 hours (all targets run concurrently)
# Requires: Enough CPU cores to run all targets (recommended: 12+ cores)
#
# Usage:
...
JOBS_PER_TARGET=$((TOTAL_CORES / 6))  # Divide cores among 6 targets
...
for TARGET in "${TARGETS[@]}"; do
    CORPUS_COUNT=$(ls "fuzz/corpus/$TARGET" 2>/dev/null | wc -l | xargs)
    ARTIFACTS=$(ls "fuzz/artifacts/$TARGET" 2>/dev/null | wc -l | xargs)
