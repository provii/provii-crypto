// SAFETY: ZK circuit constant generation. All arithmetic is finite field operations
// (BLS12-381 scalar field, Jubjub curve point arithmetic); cannot overflow.
// The `.expect()` calls are on generator points that are mathematically guaranteed
// to be valid (Sapling Pedersen generators never land on the point at infinity).
#![allow(
...
use sapling_crypto::constants::{
    NOTE_COMMITMENT_RANDOMNESS_GENERATOR as BASE_NOTE_COMMITMENT_RANDOMNESS_GENERATOR,
    NULLIFIER_POSITION_GENERATOR as BASE_NULLIFIER_POSITION_GENERATOR,
    PEDERSEN_HASH_CHUNKS_PER_GENERATOR, PEDERSEN_HASH_GENERATORS,
    PROOF_GENERATION_KEY_GENERATOR as BASE_PROOF_GENERATION_KEY_GENERATOR,
    SPENDING_KEY_GENERATOR as BASE_SPENDING_KEY_GENERATOR,
    VALUE_COMMITMENT_RANDOMNESS_GENERATOR as BASE_VALUE_COMMITMENT_RANDOMNESS_GENERATOR,
    VALUE_COMMITMENT_VALUE_GENERATOR as BASE_VALUE_COMMITMENT_VALUE_GENERATOR,
};
...
/// Number of chunks required to represent a full scalar during fixed-base exponentiation.
...
lazy_static! {
    pub static ref PROOF_GENERATION_KEY_GENERATOR: FixedGeneratorOwned =
        generate_circuit_generator(BASE_PROOF_GENERATION_KEY_GENERATOR);

    pub static ref NOTE_COMMITMENT_RANDOMNESS_GENERATOR: FixedGeneratorOwned =
        generate_circuit_generator(BASE_NOTE_COMMITMENT_RANDOMNESS_GENERATOR);

    pub static ref NULLIFIER_POSITION_GENERATOR: FixedGeneratorOwned =
        generate_circuit_generator(BASE_NULLIFIER_POSITION_GENERATOR);

    pub static ref VALUE_COMMITMENT_VALUE_GENERATOR: FixedGeneratorOwned =
        generate_circuit_generator(BASE_VALUE_COMMITMENT_VALUE_GENERATOR);

    pub static ref VALUE_COMMITMENT_RANDOMNESS_GENERATOR: FixedGeneratorOwned =
        generate_circuit_generator(BASE_VALUE_COMMITMENT_RANDOMNESS_GENERATOR);

    pub static ref SPENDING_KEY_GENERATOR: FixedGeneratorOwned =
        generate_circuit_generator(BASE_SPENDING_KEY_GENERATOR);
...
/// Create the 3-bit window table `[0, 1, ..., 8]` for different magnitudes of a fixed generator.
pub fn generate_circuit_generator(mut gen: jubjub::SubgroupPoint) -> FixedGeneratorOwned {
    ...
    for _ in 0..FIXED_BASE_CHUNKS_PER_GENERATOR {
        ...
        // Advance to the next window (equivalent to multiplying by 8).
        gen = g;
    }
...
pub(crate) fn to_montgomery_coords(g: ExtendedPoint) -> Option<(Scalar, Scalar)> {
    ...
    if y == Scalar::one() {
        ...
        if x.is_zero_vartime() {
            ...
            // INVARIANT: y != 1 because the only point with y=1 is the neutral element,
            // which is excluded by the caller (Pedersen generators are never the identity).
            let u = (Scalar::one() + y)
                * Option::<Scalar>::from((Scalar::one() - y).invert())
                    .expect("BUG: mathematically guaranteed generator point (1-y != 0)");
            // INVARIANT: x != 0 is guaranteed by the if-guard above (x.is_zero_vartime()).
            let v = u * Option::<Scalar>::from(x.invert())
                .expect("BUG: mathematically guaranteed generator point (x != 0)");
    ...
    PEDERSEN_HASH_GENERATORS
        ...
        .map(|mut gen| {
            ...
            for _ in 0..PEDERSEN_HASH_CHUNKS_PER_GENERATOR {
                // Create the (x, y) coefficients for this chunk.
                let mut coeffs = vec![];
                let mut g = gen;

                // Collect g, 2g, 3g, and 4g.
                for _ in 0..4 {
                    coeffs.push(
                        to_montgomery_coords(g.into())
                            .expect("INVARIANT: Pedersen generator multiples never land on the point at infinity"),
                    );
                    g += gen;
                }
                windows.push(coeffs);

                // Separate chunks by two doublings to prevent overlap.
                for _ in 0..4 {
                    gen = gen.double();
                }
            }
